...
def fetch_demand(region: str, start: datetime, end: datetime) -> Iterator[dict]:
    """Yield hourly demand records for a region between start and end (UTC).

    EIA returns records with: period (timestamp), respondent (region code),
    type (D=demand), value (MWh), value-units."""
...
def get_last_timestamp(con: duckdb.DuckDBPyConnection, region: str) -> datetime | None:
    ...
    row = con.execute(
        "SELECT MAX(period_utc) FROM raw_eia.raw_demand WHERE respondent = ?",
        [region],
    ).fetchone()
...
def ingest_region(con: duckdb.DuckDBPyConnection, region: str, start: datetime, end: datetime) -> int:
    ...
    con.executemany("""
        INSERT OR IGNORE INTO raw_eia.raw_demand
